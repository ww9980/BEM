...
size=500;
 ...
 //     }
 //   }
 //   else {			/* Isn't an op, must be the input file. */
 ...
 // printf("  Depth of tree selected: %i\n",numLev);
  ...
  i=0;
	for(xx=0;xx<=19;xx++){
		for(yy=0;yy<=18;yy++){
			...
			poten[i]=(xx>=9 && xx<=11 && yy>=9 && yy<=11)?1:0;
			dbydnpoten[i]=0;
			type[i]=DIRICHLET;
			i++;
		}
  ...
  for(i=0;i<size;i++) fprintf(stdout,"%f\n",lhsvect[i]);
...
i=0;
 ...
 for(i=0; i<size; i++) {
  ...
  nlhs=370;
  for(i=0;i<nlhs;i++){
	xcoll[i*DIMEN+0]=i*0.1+0.5;
	xcoll[i*DIMEN+1]=i*0.1+0.5;
	xcoll[i*DIMEN+2]=0;
  }
  numit = fastlap(&nlhs,&size,&size,x,shape,dtype,lhstype,rhstype,lhsindex,rhsindex,lhsvect,rhsvect,xcoll,xnrm,&numLev,&numMom,&maxit,&tol,&fljob);

  for(i=0;i<nlhs;i++) 
	  
	  fprintf(stdout,"%f\n",lhsvect[i]/2./3.14159265358979323846264338328);
  ////////////////
