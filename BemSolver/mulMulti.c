...
/*
  ...
  used for all cases except for the sine part of arrays (use sindex_() instead)
...
int order;
{
  ...
  vector[index_(0, 0)] = 1.0;	/* P0^0 */
  if(order > 0) {
    vector[index_(1, 0)] = cosA;	/* P1^0 */
    vector[index_(1, 1)] = sinMA = -sqrt(1-cosA*cosA); /* P1^1 = -sin(alpha) */
  }
  if(order > 1) vector[index_(2, 1)] = 3*sinMA*cosA; /* P2^1 = -3sin()cos() */
  ...
  for(m = 0; m < order+1; m++) {
    if(m != 0 && m != 1) {	/* set up first two evaluations in row */
      ...
      /* use recursion on m */
      if(vector[index_(1, 1)] == 0.0) {
	vector[index_(m, m)] = 0.0;
	if(m != order) vector[index_(m+1, m)] = 0.0;	/* if not last row */
      }
      else {
	cosA = vector[index_(1,0)]/vector[index_(1,1)]; /* cosA= -cot(theta) */
	sinMA *= vector[index_(1,1)]; /*(-sin(alpha))^(m-1)->(-sin(alpha))^m*/
	vector[index_(m, m)] = fact * sinMA;
	if(m != order) {		/* do if not on last row */
	  vector[index_(m+1, m)] = vector[index_(1, 0)]*(2*m+1)
	      *vector[index_(m, m)];
	}
      }
    }
    for(x = 2; x < order-m+1; x++) { /* generate row of evals recursively */
      vector[index_(x+m, m)] = 
	  ((2*(x+m)-1)*vector[index_(1, 0)]*vector[index_(x+m-1, m)]
	   - (x + 2*m - 1)*vector[index_(x+m-2, m)])/x;
    }
  }
}
	...
	for(m = 0, mBeta = 0.0; m <= n; m++, mBeta += beta) {
	  ...
	  if(k == 0) {		/* figure terms for Nj^0, ie k = 0 */
	    if(m <= j-n) {	/* if O moments are nonzero */
	      temp1 = fact(j)*rhoPwr*iPwr(2*m)*tleg[index_(n, m)];
	      temp1 /= (fact(j-n+m)*fact(n+m));
	      mat[index_(j, k)][index_(j-n, m)] += temp1*cos(mBeta);
	      if(m != 0)		/* if sin term is non-zero */
		  mat[index_(j, k)][sindex_(j-n, m, cterms)] += temp1*sin(mBeta);
	    }
	  }
	  else {		/* figure terms for Nj^k, k != 0 */
	    temp1 = fact(j+k)*rhoPwr*tleg[index_(n, m)]/fact(n+m);
		...
		mat[index_(j, k)][index_(j-n, m-k)] += temp1*cos(mBeta);
		mat[index_(j, k)][sindex_(j-n, m-k, cterms)] += temp1*sin(mBeta);
	      }
	      else if(k-m == 0) {	/* double to compensate for 2Re sub. */
		mat[index_(j, k)][index_(j-n, k-m)] += 2*temp1*cos(mBeta);
		/* sin term is always zero */
	      }
	      else if(k-m > 0 && k-m <= j-n) {
		mat[index_(j, k)][index_(j-n, k-m)] += temp1*cos(mBeta);
		mat[index_(j, k)][sindex_(j-n, k-m, cterms)] -= temp1*sin(mBeta);
	      }
	      if(k+m <= j-n) {
		mat[index_(j, k)][index_(j-n, k+m)] += temp2*cos(mBeta);
		mat[index_(j, k)][sindex_(j-n, k+m, cterms)] += temp2*sin(mBeta);
	      }
	    }			/* do if m = 0 and O moments not zero */
	    else if(k <= j-n) mat[index_(j, k)][index_(j-n, k)] += temp2;

	    /* write the sin(kPhi) coeff, dblbar(N)j^k, if it is non-zero */
	    if(m != 0) {
	      if(k-m < 0 && abs(k-m) <= j-n) {	/* use conjugates here */
		mat[sindex_(j, k, cterms)][index_(j-n, m-k)] += temp1*sin(mBeta);
		mat[sindex_(j, k, cterms)][sindex_(j-n, m-k, cterms)] 
		    -= temp1*cos(mBeta);
	      }
	      else if(k-m == 0) {/* double to compensate for 2Re sub */
		mat[sindex_(j, k, cterms)][index_(j-n, k-m)] 
		    += 2*temp1*sin(mBeta);
		/* sine term is always zero */
	      }
	      else if(k-m > 0 && k-m <= j-n) {
		mat[sindex_(j, k, cterms)][index_(j-n, k-m)] += temp1*sin(mBeta);
		mat[sindex_(j, k, cterms)][sindex_(j-n, k-m, cterms)] 
		    += temp1*cos(mBeta);
	      }
	      if(k+m <= j-n) {
		mat[sindex_(j, k, cterms)][index_(j-n, k+m)] -= temp2*sin(mBeta);
		mat[sindex_(j, k, cterms)][sindex_(j-n, k+m, cterms)] 
		    += temp2*cos(mBeta);
	      }
	    }			/* do if m = 0 and moments not zero */
	    else if(k <= j-n) mat[sindex_(j, k,cterms)][sindex_(j-n, k, cterms)] 
		+= temp2;
	  }
	}
      }
...
int numfpts, order;
{
  ...
  for(n = 1; n <= order; n++) {
    for(m = 1; m <= n; m++) {
      for(i = 0; i < numfpts; i++) mat[i][index_(n, m)] /= factFac[n][m];
    }
  }
  ...
  for(i = 0; i < numfpts; i++) { /* loop on rows of matrix */
	...
	mat[i][sindex_(n, m, cterms)] = mat[i][index_(n, m)];
      }
    }
  }
  ...
  for(m = 1; m <= order; m++) {	/* lp on m in Mn^m (no m=0 since cos(0)=1) */
	...
	mat[i][index_(n, m)] *= cos(Mphi[i]);
	mat[i][sindex_(n, m, cterms)] *= sin(Mphi[i]);
      }
    }
  ...
  for(i = 0; i < numfpts; i++)
    if(fpts[i]->deriv == 1) {
      for(n = 0; n < order; n++) {
        /* m=0 */
        factorial = (n+2)*(n+1);
        matP1 = factorial*mat[i][index_(n+1, 1)];
        matP2 = factorial*mat[i][sindex_(n+1, 1, cterms)];
        matC1 = (n+1)*mat[i][index_(n+1, 0)];
        tmp  = fpts[i]->nrm[0]*matP1;
        tmp += fpts[i]->nrm[1]*matP2;
        tmp -= fpts[i]->nrm[2]*matC1;
        mat[i][index_(n, 0)] = tmp;
        for(m = 1; m <= n; m++) {
          matM1 = mat[i][index_(n+1, m-1)];
          if (m == 1)
            matM2 = 0.0;
          else
            matM2 = mat[i][sindex_(n+1, m-1, cterms)];

          factorial = (n+m+2)*(n+m+1);
          matP1 = factorial*mat[i][index_(n+1, m+1)];
          matP2 = factorial*mat[i][sindex_(n+1, m+1, cterms)];
          matC1 = (n+1+m)*mat[i][index_(n+1, m)];
          matC2 = (n+1+m)*mat[i][sindex_(n+1, m, cterms)];            

          tmp  = fpts[i]->nrm[0]*0.5*(matP1-matM1);
          tmp += fpts[i]->nrm[1]*0.5*(matP2+matM2);
          tmp -= fpts[i]->nrm[2]*matC1;
          mat[i][index_(n, m)] = tmp;
          tmp  = fpts[i]->nrm[0]*0.5*(matP2-matM2);
          tmp -= fpts[i]->nrm[1]*0.5*(matP1+matM1);
          tmp -= fpts[i]->nrm[2]*matC2;
          mat[i][sindex_(n, m, cterms)] = tmp;
        }
      }
      mat[i][index_(order, 0)] = 0.0;
      for(m = 1; m <= order ; m++) {
        mat[i][index_(order, m)] = 0.0;
        mat[i][sindex_(order, m, cterms)] = 0.0;
      }
    }
