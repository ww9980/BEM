  ...
  for(j = 0, rhoJ = 1.0; j <= order; rhoJ *= rho, j++) {
	...
	for(m = 0; m <= n; m++) {  /* loop on On^m's, multipole moments */
	  ...
	  if(k == 0) {	       /* use abbreviated formulae in this case */
	    ...
	    if(m != 0) {
	      temp1 = tleg[index_(j+n, m)]*facFrA[j+n-m][n+m];
	      mat[index_(j, 0)][index_(n, m)] += temp1*cosB(m)*rhoFac;
	      mat[index_(j, 0)][sindex_(n, m, ct)] += temp1*sinB(m)*rhoFac;
	    }
	    else mat[index_(j, 0)][index_(n, 0)] 
		+= tleg[index_(j+n, 0)]*facFrA[j+n][n]*rhoFac;
	  }
	  else {
	    temp1 = tleg[index_(j+n, abs(m-k))]
		*facFrA[j+n-abs(m-k)][n+m]*iPwr(abs(k-m)-k-m);
	    temp2 = tleg[index_(j+n, m+k)]*facFrA[j+n-m-k][n+m];
	    temp3 = tleg[index_(j+n, k)]*facFrA[j+n-k][n]*2;

	    /* generate bar(N)j^k entry */
	    if(m != 0) {
	      mat[index_(j, k)][index_(n, m)] 
		  += (temp1*cosB(m-k)+temp2*cosB(m+k))*rhoFac;
	      mat[index_(j, k)][sindex_(n, m, ct)] 
		  += (temp1*sinB(m-k)+temp2*sinB(m+k))*rhoFac;
	    }
	    else mat[index_(j, k)][index_(n, 0)] += temp3*cosB(k)*rhoFac;

	    /* generate dblbar(N)j^k entry */
	    if(m != 0) {
	      mat[sindex_(j, k, ct)][index_(n, m)] 
		  += (-temp1*sinB(m-k)+temp2*sinB(m+k))*rhoFac;
	      mat[sindex_(j, k, ct)][sindex_(n, m, ct)] 
		  += (temp1*cosB(m-k)-temp2*cosB(m+k))*rhoFac;
	    }
	    else mat[sindex_(j, k, ct)][index_(n, 0)] += temp3*sinB(k)*rhoFac;
	  }
	}
      }
    }
  ...
  for(j = 0, rhoJ = 1.0; j <= order; rhoJ *= (-rho), j++) {
	...
	for(m = 0; m <= n; m++) { /* loop on On^m's, old local moments */
	  ...
	  if(k == 0 && n-j >= m) {  /* use abbreviated formulae in this case */
	    ...
	    /* generate only bar(N)j^0 entry (dblbar(N)j^0 = 0 always) */
	    if(m != 0) {
	      temp1 = tleg[index_(n-j, m)]*facFrA[0][n-j+m]*rhoFac;
	      mat[index_(j, 0)][index_(n, m)] += temp1*cosB(m);
	      mat[index_(j, 0)][sindex_(n, m, ct)] += temp1*sinB(m);
	    }
	    else mat[index_(j, 0)][index_(n, 0)] += tleg[index_(n-j, 0)]
		    *facFrA[0][n-j]*rhoFac;
	  }
	  else {
	    if(n-j >= abs(m-k)) temp1 = tleg[index_(n-j, abs(m-k))]
		*facFrA[0][n-j+abs(m-k)]*iPwr(m-k-abs(m-k))*rhoFac;
	    if(n-j >= m+k) temp2 = tleg[index_(n-j, m+k)]
		*facFrA[0][n-j+m+k]*iPwr(2*k)*rhoFac;
	    if(n-j >= k) temp3 = 2*tleg[index_(n-j, k)]
		*facFrA[0][n-j+k]*iPwr(2*k)*rhoFac;

	    /* generate bar(N)j^k entry */
	    if(m != 0) {
	      if(n-j >= abs(m-k)) {
		mat[index_(j, k)][index_(n, m)] += temp1*cosB(m-k);
		mat[index_(j, k)][sindex_(n, m, ct)] += temp1*sinB(m-k);
	      }
	      if(n-j >= m+k) {
		mat[index_(j, k)][index_(n, m)] += temp2*cosB(m+k);
		mat[index_(j, k)][sindex_(n, m, ct)] += temp2*sinB(m+k);
	      }
	    }
	    else if(n-j >= k) mat[index_(j, k)][index_(n, 0)] += temp3*cosB(k);

	    /* generate dblbar(N)j^k entry */
	    if(m != 0) {
	      if(n-j >= abs(m-k)) {
		mat[sindex_(j, k, ct)][index_(n, m)] += (-temp1*sinB(m-k));
		mat[sindex_(j, k, ct)][sindex_(n, m, ct)] += temp1*cosB(m-k);
	      }
	      if(n-j >= m+k) {
		mat[sindex_(j, k, ct)][index_(n, m)] += (-temp2*sinB(m+k));
		mat[sindex_(j, k, ct)][sindex_(n, m, ct)] += temp2*cosB(m+k);
	      }
	    }
	    else if(n-j >= k) 
		mat[sindex_(j, k, ct)][index_(n, 0)] += temp3*sinB(k);
	  }
	}
      }
    }
...
int numfpts, order;
{
  ...
  for(n = 0; n <= order; n++) {
    for(m = 0; m <= n; m++) {
      for(i = 0; i < numfpts; i++) mat[i][index_(n, m)] /= fact(n+m);
    }
  }
...
/*cdel nr_mulLocal_calls++;*/
  ...
  for(i = 0; i < numfpts; i++) {
    if(fpts[i]->deriv == 1) {
      for(n = order; n >= 2; n--) {
        /* m=0 */
        matP1 = mat[i][index_(n-1, 1)];
        matP2 = mat[i][sindex_(n-1, 1, cterms)];
        matC1 = mat[i][index_(n-1, 0)];
        ...
        for(m = 1; m <= n ; m++) {
          ...
          if (m == n) {
            ...
            matC1 = mat[i][index_(n-1, m)];
            matC2 = mat[i][sindex_(n-1, m, cterms)];
            if (m == n-1) {
              matP1 = 0.0;
              matP2 = 0.0;
            }
            else {
              matP1 = mat[i][index_(n-1, m+1)];
              matP2 = mat[i][sindex_(n-1, m+1, cterms)];
            }
          }
      ...
      /* n = 1 */
      matC1 = mat[i][index_(0, 0)];
      mat[i][index_(1, 0)] = fpts[i]->nrm[2]*matC1;
      mat[i][index_(1, 1)] = -0.5*fpts[i]->nrm[0]*matC1;
      mat[i][sindex_(1, 1, cterms)] = -0.5*fpts[i]->nrm[1]*matC1;
      /* n = 0 */
      mat[i][index_(0,0)] = 0.0;
    }
  }    
