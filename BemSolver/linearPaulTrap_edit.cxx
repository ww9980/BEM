...
void ExportAxialPot(double zstart=-15,double zstop=15,double zcnt=3000){
	int elnum,cnt;
	double zpos;
	double *pot=new double[(int)zcnt];
	double *xaxis=new double[(int)zcnt];
...
void PlotPseudo(){
		...
		double mass=40.078*1.66e-27;
		double q=1.602e-19;
		double OmegaRF=20000000;
		double Urf=100;
		setZero(0,0,0,0,Urf);
		const int num=50;
		double *pot=new double[num*num];
		double *Ex=new double[num*num];
		double *Ey=new double[num*num];
		double *Ez=new double[num*num];
		double *xxx=new double[num*3*num];
		double *xaxis=new double[num*num];
		double *yaxis=new double[num*num];
		...
		for(x=0;x<num;x++){
			for(y=0;y<num;y++) {
				//double val=wr.calc(0.25,0.25,xstart+(-xstart+xstop)/double(num)*x);
				//cut.SetPoixt(cxt++,start+(-start+stop)/double(num)*x,val );
				
				xxx[n*3+0]=xstart+(-xstart+xstop)/double(num-1)*x;;
				xxx[n*3+1]=ystart+(-ystart+ystop)/double(num-1)*y;
				xxx[n*3+2]=zpos;

				xaxis[n]=xxx[n*3+0];
				yaxis[n]=xxx[n*3+1];
					wr->calc(xxx[n*3+0],xxx[n*3+1],xxx[n*3+2],
					 pot[n],Ex[n],Ey[n],Ez[n]);
				//result[x]=val;
		...
		double Ex0,Ey0,Ez0,pot0;
		wr->calc(0,0,0, pot0,Ex0,Ey0,Ez0);
		cout <<(Ex0*Ex0-Ey0*Ey0-Ez0*Ez0)/(4.*mass*OmegaRF);
		int i;
		double *pseudo=new double[num*num];
		for(i=0;i<num*num;i++) pseudo[i]=q*(Ex[i]*Ex[i]+Ey[i]*Ey[i]+Ez[i]*Ez[i]-Ex0*Ex0-Ey0*Ey0-Ez0*Ez0)/(4.*mass*OmegaRF*OmegaRF);
		logfile.WriteTSV("./xaxis",xaxis,num*num);
		logfile.WriteTSV("./yaxis",yaxis,num*num);

		logfile.WriteTSV("./seudo",pseudo,num*num);

		//TGraph2D potplot(num*num,xaxis,yaxis,pseudo);
		//gStyle->SetPalette(1);
		//potplot.Draw("surf1");
...
void Trajectory(double rfac=0.,double rfbc=0.,double dcac=0.,double dcbc=0.,double x=0.00002,double y=0.00002,double z=0.0,double vx=0,double vy=0,double vz=0,double stoptime=1000./12e6,int steps=500000,double RFVoltage=400./2.,double RFfreq=12155000,bool verlet=false){
	...
	for(i=0;i<steps;++i){
		...
		tpos[i]=t;
		if(verlet) wr->propagateForwardVerlet(xx,vv,h,qDivM);
		else wr->propagateForwardEuler(xx,vv,h,qDivM);
		//
		t+=h;
	}
	...
	logfile.Graph("x-y-pos",xpos,ypos,steps,"x [mm]","y [mm]","AL");
	logfile.WriteTSV("x-y-pos_verlet",xpos,ypos,steps);
