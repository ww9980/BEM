...
int order;
{
  ...
  if (pp->rhsType == POINT_SOURCE) {
    ...
    fprintf(stderr,"FLE-calcpsph: unknown point singularity type: %d\n",pp->lhsType);
  }
...
int order;
{
  ...
  if (pp->rhsType == CONSTANT_SOURCE) {
    ...
    fprintf(stderr,"FLE-calcpsph: unknown singularity type: %d\n",pp->lhsType);
  }
  ...
  for(n = 0; n <= order; n++) {
    multiM[index_(n,0)] = Mrmn[0][n];
    /*flip dipole sign here to agree with the panel normal convention.*/
    multiD[index_(n,0)] = -Nrmn[0][n];
    for(m = 1; m <= n; m++) {
      multiM[index_(n,m)] = Mrmn[m][n];
      multiM[sindex_(n,m,rterms)] = Mrtmn[m][n];
      /*flip dipole sign here to agree with the panel normal convention.*/
      multiD[index_(n,m)] = -Nrmn[m][n];
      multiD[sindex_(n,m,rterms)] = -Nrtmn[m][n];
    }
  } 
}
