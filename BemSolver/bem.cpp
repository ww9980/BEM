...
//#include <atltime.h> //ATL DOES NOT WORK with vc2005express
#include "TSystem.h"
#include "TROOT.h"
#include "ExpLog.h"
#undef StrDup;
...
void D3element::InsertTGeoVolume(TGeoVolume *top,TGeoMedium *matVak,TGeoMedium *mat,TGeoManager *geom){
#define HIRARCHY3D
#ifdef HIRARCHY3D
	TGeoVolume *R = geom->MakeBox("R", matVak, 0., 0., 0.);
    R->SetVisibility(kFALSE);
	TGeoCombiTrans *combi = new TGeoCombiTrans(x,y,z, 
                                   new TGeoRotation("rot2",-psi*180./pi,-theta*180./pi,-phi*180./pi));
	top->AddNode(R,1,combi);
#else
	TGeoVolume *R=top;
#endif
	...
	for(i=subelement.begin();i!=subelement.end();++i){
		(*i)->InsertTGeoVolume(R,matVak,mat,geom);
	}
};
...
void D3rectangle::InsertTGeoVolume(TGeoVolume *top,TGeoMedium *matVak,TGeoMedium *mat,TGeoManager *geom){
	if(!isBaseElement){
		TGeoVolume *R = geom->MakeBox("R", matVak, 0., 0., 0.);
		R->SetVisibility(kFALSE);
		TGeoCombiTrans *combi = new TGeoCombiTrans(x,y,z, 
		...
		TGeoVolume *R = geom->MakeBox("R", matVak, 0., 0., 0.);
		R->SetVisibility(kFALSE);
		TGeoCombiTrans *combi0 = new TGeoCombiTrans(x,y,z, 
									new TGeoRotation("rot2",-psi*180./pi,-theta*180./pi,-phi*180./pi));
		top->AddNode(R,1,combi0);


#ifdef HIRARCHY3D
			TGeoTranslation *combi = new TGeoTranslation("trans0",0,0,0);
                                   
#else
		TGeoCombiTrans *combi = new TGeoCombiTrans(h);
#endif
		TGeoVolume *bar3 = geom->MakeArb8("bar3", mat, 1e-19);
		bar3->SetLineColor(Color);
		TGeoArb8 *arb = (TGeoArb8*)bar3->GetShape();
		arb->SetVertex(0, 0., 0.);
		arb->SetVertex(1, xa, ya);
		arb->SetVertex(2, xb, yb);
		arb->SetVertex(3, xc, yc);
		arb->SetVertex(4, 0., 0.);
		arb->SetVertex(5, xa, ya);
		arb->SetVertex(6, xb, yb);
		arb->SetVertex(7, xc, yc);
		R->AddNode(bar3, 1, combi);



#ifdef HIRARCHY3D
		// a x b;
		double nz=-ya*xb + xa*yb;
		TGeoTranslation *combi2 = new TGeoTranslation("trans1",0,0,((nz>0)^inversenorm)?-epsilon:epsilon);
                                   
#else
		TGeoCombiTrans *combi2 = new TGeoCombiTrans(h);
#endif
		TGeoVolume *bar4 = geom->MakeArb8("bar4", mat, 1e-19);
		bar4->SetLineColor(kRed);
		TGeoArb8 *arb1 = (TGeoArb8*)bar4->GetShape();
		arb1->SetVertex(0, 0., 0.);
		arb1->SetVertex(1, xa, ya);
		arb1->SetVertex(2, xb, yb);
		arb1->SetVertex(3, xc, yc);
		arb1->SetVertex(4, 0., 0.);
		arb1->SetVertex(5, xa, ya);
		arb1->SetVertex(6, xb, yb);
		arb1->SetVertex(7, xc, yc);
		R->AddNode(bar4, 1, combi2);
	}
}
...
void D3triangle::InsertTGeoVolume(TGeoVolume *top,TGeoMedium *matVak,TGeoMedium *mat,TGeoManager *geom){
	if(!isBaseElement){
		TGeoVolume *R = geom->MakeBox("R", matVak, 0., 0., 0.);
		R->SetVisibility(kFALSE);
		TGeoCombiTrans *combi = new TGeoCombiTrans(x,y,z, 
...
//   //--- define some media
   ...
   geom->SetTopVolume(top);
	...
	PD3elementList::iterator i;
	i=subelement.begin();
	//if(i!=subelement.end()) ++i;
	for(;i!=subelement.end();++i){
		(*i)->InsertTGeoVolume(top,Vacuum,Al,geom);
	}
